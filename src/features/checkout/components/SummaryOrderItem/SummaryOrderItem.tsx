...
const SummaryOrderItem: React.FC<SummaryOrderItemProps> = ({ item }) => {
  ...
  return (
    <div className={`flex align-item-center`} key={id}>
      <img className={`${styles.image} mr-10`} src={image} alt={title} />
      <div className="flex-grow">
        <p className="fs-16 fw-medium mb-5">{title}</p>
        <p className="fs-14 text-muted">${price.toFixed(2)} X {quantity}</p>
      </div>
      <div className="fs-16 fw-bold text-primary">${(price * quantity).toFixed(2)}</div>
    </div>
  );
};
